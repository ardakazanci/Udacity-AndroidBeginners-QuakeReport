...
public final class Araclar {
    // Örnek API istek çıktısı - Static Final olarak belirtildi.
    ...
    // Private olmasının sebebi , Araclar sınıfından nesne oluşturulmayacaktır. Buna izin verilmemelidir.
    // Bu sınıf sadece erişilebilen static değişkenleri ve yöntemleri tutmaktadır.
    // Doğrudan sınıf adı ile iletişim kurulabilir - Bu yüzden private ' tir. Static ve final değişkenler tanımlanmıştır.
    private Araclar() {

    }


    // Parse edilen değerler return olarak bir list dönecek
    public static final ArrayList<Model> parseEdilenVeriler() {

        ArrayList<Model> depremListesi = new ArrayList<Model>(); // Model içeriğinden değerler alabilen list doldurulacak
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                JSONObject jsonOzellik = jsonArray.getJSONObject(i);

                JSONObject jsonOzellikProperties = jsonOzellik.getJSONObject("properties");

                double depremSiddet = jsonOzellikProperties.getDouble("mag");
                String depremLokasyon = jsonOzellikProperties.getString("place");
                long depremZaman = jsonOzellikProperties.getLong("time"); // long türüne çevirildiği için değişim sağlandı.
                String depremUrl = jsonOzellikProperties.getString("url");

                depremListesi.add(new Model(depremSiddet, depremLokasyon, depremZaman,depremUrl));
            ...
            e.printStackTrace();
            Log.e("Araclar.class", "JSON Parse işleminde bir istisna yaşandı");

        }


        return depremListesi;
